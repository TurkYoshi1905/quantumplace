...
Kuantum temalı, işbirlikçi bir piksel boyama oyunu. r/place'ten ilham alınarak React 19.2, TypeScript ve Tailwind CSS ile sıfırdan geliştirilmiştir.
...
Betik, Replit monorepo yapısını değil yalnızca gerçek uygulama dosyalarını (src/, public/, index.html, vb.) gönderir. `git push --force` kesinlikle kullanılmaz.
...
- **Resim 1 (Uzak Görünüm):** Oyun ilk açıldığında 2048×2048 piksel tuval ekrana sığdırılarak tüm alan görüntülenir.
- **Resim 2 (Yakın Görünüm):** Maksimum 5x büyütme, fare imlecini takip eden köşe braket biçiminde crosshair/nişan göstergesi.
...
### Canvas ve Etkileşim
- 2048×2048 piksel HTML5 Canvas üzerinde tam render
...
- Tüm piksel verisi (2048×2048 × 4 byte = ~16MB) tarayıcının IndexedDB veritabanına kaydedilir.
...
- **Offscreen canvas:** Pixel verisini 16MB Uint8ClampedArray'de tutar; her render'da `drawImage()` ile ekran canvas'ına kopyalar.
...
- Fonlar, piksel verisi, cooldown: production-ready düzeyinde uygulanmıştır.
